...
def run_training_experiment(epsilon):
    ...
    central_privacy = parse_mechanism(
        ...
        noise_cohort_size=5000,  # variable for this experiment
        num_epochs=100,
        population=1e6,
        min_separation=None,
        is_central=True
    ...
    callbacks = [
        ...
        AggregateMetricsToDisk(f'./metrics_epsilon_6/{epsilon}.csv'),
        # Set checkpoint frequency equal to 50 (i.e., only at the final epoch)
        HuggingFaceModelCheckpointingCallback(hf_model, tokenizer, f"hf_checkpoint/pfl_epsilon_6/{epsilon}", checkpoint_frequency=10)
    ]
...
# clipping_bounds = [0.01, 0.05, 0.1, 0.5, 1.0, 10.0]
for nc in epsilons:
    logger.info(f"Running experiment with epsilon = {nc}")
    # The output directory for checkpointing can include the noise cohort value.
    final_model = run_training_experiment(epsilon=nc)
