...
with open("dataset/easy_eval_data.json", "r") as f:
...
def takeCheckPoint(model_path, tokenizer, peft_model, csv_writer):
    ...
    for idx, example in enumerate(tqdm(hf_eval_dataset, desc="Processing Examples")):
        ...
        batched_examples.append((example, sentence_tokens, true_label_id, true_targets))
        ...
        if len(batched_inputs) == batch_size or idx == len(hf_eval_dataset) - 1:
            ...
            for output, prompt, (example, sentence_tokens, true_label_id, true_targets) in zip(outputs, batched_prompts, batched_examples):
...
#Evaluating only one experiment
def iterateCheckPoints(output_csv="pfl_final_best/4.csv", checkpoints_dir = "checkpoints/mistral_attention_new"): #ddit
    output_csv="fl_final_5.csv"
    checkpoints_dir = "pfl-research/benchmarks/hf_checkpoint/final_fl/final_fl" #edit
     #edit
    ...
    # Iterate through each checkpoint
        for checkpoint in sorted(os.listdir(checkpoints_dir)):
            ...
            if os.path.isdir(checkpoint_path):
                ...
                takeCheckPoint(checkpoint_path, tokenizer, peft_model, csv_writer)

def iterateCheckPoints_2(output_csv="pfl_evaluation_clipping_bound_6.csv"): #ddit
    checkpoints_dir = "pfl-research/benchmarks/hf_checkpoint/clipping_bounds/pfl_clipping_bound_6" #edit
...
def iterateCheckPoints_3():
        checkpoint_path_directory =  "pfl-research/benchmarks/hf_checkpoint/final_pfl_best"
        iteration = 0
        for checkpoint in sorted(os.listdir(checkpoint_path_directory)):
            if checkpoint.startswith("."):
                continue
            checkpoint_path = os.path.join(checkpoint_path_directory, checkpoint)
            iterateCheckPoints(output_csv = f'pfl_final_best/{iteration}.csv', checkpoints_dir = checkpoint_path)
            iteration +=1
